...
app.delete('/api/articles/:id', (q, s) => { const d = read(); d.articles = d.articles.filter(x => x.id !== q.params.id); write(d); s.json({ ok: true }); });
...
app.post('/api/posts/:id/approve', (q, s) => { const d = read(), p = d.posts.find(x => x.id === q.params.id); if (!p) return s.status(404).json({ error: 'not found' }); p.status = 'approved'; d.articles.unshift({ id: 'post-' + p.id, title: p.title, description: p.body, category: p.category, image: p.image, source: 'Community', publishedAt: new Date().toISOString() }); write(d); s.json(p); });
app.post('/api/posts/:id/reject', (q, s) => { const d = read(), p = d.posts.find(x => x.id === q.params.id); if (!p) return s.status(404).json({ error: 'not found' }); p.status = 'rejected'; write(d); s.json(p); });
app.post('/api/ticker', (q, s) => { const d = read(); d.ticker = q.body.ticker || d.ticker; write(d); s.json({ ticker: d.ticker }); });
...
app.post('/api/upload', upload.single('image'), (q, s) => { if (!q.file) return s.status(400).json({ error: 'image required' }); s.json({ url: '/uploads/' + q.file.filename }); });
...
app.get('/article/:id', (q, s) => s.sendFile(path.join(__dirname, 'public', 'article.html')));
app.get('/sitemap.xml', (q, s) => { const d = read(); const baseUrl = process.env.SITE_URL || `${q.protocol}://${q.get('host')}`; const urls = ['/'].concat((d.articles || []).map(a => '/article/' + encodeURIComponent(a.id))); const xml = '<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">' + urls.map(u => `<url><loc>${baseUrl}${u}</loc></url>`).join('') + '</urlset>'; s.type('application/xml').send(xml); });
